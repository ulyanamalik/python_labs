        ...
        if rows:
            ...
            for row in rows:
                if len(row) != first_row_len:
                    raise ValueError(f"Все строки должны иметь одинаковую длину ({first_row_len}). Получено {len(row)} элементов.")
...
# Запись CSV-файла
write_csv([("word", "count"), ("test", 3)], r"C:\Users\1\Documents\GitHub\ulyana\data\check.csv")