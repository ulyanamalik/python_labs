...
import re
@dataclass #Что делает: Автоматически генерирует стандартные методы 
            #класса (__init__, __repr__, __eq__)
            #Зачем нужно: Позволяет писать меньше кода, делает класс более читаемым
            #Без dataclass пришлось бы писать вручную все эти методы
class Student:
    fio: str
    birthdate: str
    group: str
