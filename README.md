...
# Импорт Student из соседней папки
sys.path.append(str(Path(__file__).parent.parent))
from lab8.models import Student


class Group:
    # Инициализация объекта Group - создает/проверяет файл с данными
    def __init__(self, storage_path: str):
        # Запоминаем путь к файлу данных
        self.path = Path(storage_path)
        
        # Если файла нет - создаем папки и сам файл с заголовками
        if not self.path.exists():
            self.path.parent.mkdir(parents=True, exist_ok=True)
            self.path.write_text("fio,birthdate,group,gpa\n", encoding='utf-8')
        
        # Проверяем, что в существующем файле правильные заголовки
        content = self.path.read_text(encoding='utf-8').strip()
        if content and not content.split('\n')[0] == 'fio,birthdate,group,gpa':
            raise ValueError('Не корректный заголовок')

    # Вспомогательный метод: гарантирует что файл существует
    ...
    # Вспомогательный метод: читает все данные из файла
    def _read_all(self) -> List[dict]:
        self._ensure_storage_exists()
        with open(self.path, 'r', encoding='utf-8') as f:
            return list(csv.DictReader(f))

    # Возвращает список всех студентов (без заголовков)
    def list(self):
        with open(self.path, 'r', encoding='utf-8') as f:
            rd = csv.reader(f)
            next(rd)  # Пропускаем строку с заголовками
            students = list(rd)
        return students
    
    # Вспомогательный метод: записывает всех студентов в файл
    ...
    def add(self, student: Student):
        ...
        # Проверяем, нет ли студента с таким же ФИО
    ...
    # Ищет студентов по части ФИО
    def find(self, substr: str):
        with open(self.path, 'r', encoding='utf-8') as f:
            rd = list(csv.DictReader(f))
        
        # Возвращаем студентов, у которых в ФИО есть подстрока
        return [Student(r['fio'], r['birthdate'], r['group'], float(r['gpa'])) 
    ...
    def remove(self, fio: str):
        with open(self.path, 'r', encoding='utf-8') as f:
            rd = csv.DictReader(f)
            # Оставляем всех, кроме тех, у кого есть такое ФИО
    ...
    def update(self, fio: str, **fields):
        # Преобразуем GPA в число
        for key, value in fields.items():
            if key == 'gpa':
                fields[key] = float(value)
        
        # Создаем временного студента для валидации
        ...
        # Ищем студента и обновляем его данные
        with open(self.path, 'r', encoding='utf-8') as f:
            rd = list(csv.DictReader(f))
            found = False
...
```
<img width="1242" height="563" alt="Снимок экрана 2025-12-15 222039" src="https://github.com/user-attachments/assets/ffae1549-44c8-4d9d-9112-89deb718446e" />
...
```
<img width="1271" height="362" alt="Снимок экрана 2025-12-15 222717" src="https://github.com/user-attachments/assets/de71d549-66e8-41c6-ab66-5a40f72cef2c" />
...
<img width="615" height="277" alt="image" src="https://github.com/user-attachments/assets/5a8b2544-ffc2-44ac-8865-1bcaf44ce9eb" />
...
import re
@dataclass              #специальный декоратор,который автоматически
                        #создает стандартные методы для классов
                        #класса (__init__(конструктор), __repr__(строковое представление), __eq__(сравнение объектов)
class Student:
    fio: str
    birthdate: str
    group: str
    ...
    def __post_init__(self): #Вызывается автоматически после __init__(),__post_init__ - вызывается после __init__ в dataclass
        ...
        try:
            datetime.strptime(self.birthdate, "%Y-%m-%d")#Пытается преобразовать строку в объект datetime
                                                        #Проверяет, что дата существует 
                                                        #(например, 2023-02-30 вызовет ошибку)
        except ValueError:
            raise ValueError(f"Неверная дата: {self.birthdate}")
        if not (0 <= self.gpa <= 5):#Проверяет, что средний балл от 0 до 5
                                    #Гарантирует корректность данных
            raise ValueError(f"Средний балл должен быть в диапазоне от 0 до 5, получено: {self.gpa}")
        if len(self.fio.split()) < 2:#Проверяет, что ФИО содержит минимум 2 слова
                                    #Зачем нужно: Убедиться, что есть хотя бы имя и фамилия
            raise ValueError(f"ФИО должно содержать имя и фамилию: {self.fio}")

    def age(self) -> int:#Рассчитывает полный возраст студента
        ...
        if (today.month, today.day) < (birth_date.month, birth_date.day):
            age -= 1    
        return age
    def to_dict(self) -> dict:#Создает словарь с данными студента
    ...
    @classmethod    #Создает объект Student из словаря
                    #@classmethod: Метод класса (работает с классом, а не с экземпляром)
                    #data.get(key, default): Безопасно получает значение из словаря
                    #Для десериализации из JSON
    def from_dict(cls, data: dict):
        return cls(
            fio=data.get("fio", ""), #data.get(key, default):
                                    #Безопасно получает значение из словаря
            birthdate=data.get("birthdate", ""),
            group=data.get("group", ""),
            gpa=data.get("gpa", 0.0)
        )
    def __str__(self) -> str:#Что делает: Определяет, как объект будет 
                            #выглядеть при выводе через print()
                            #Для удобного отображения информации о студенте
        return f"{self.fio}, группа: {self.group}, возраст: {self.age()}, средний балл: {self.gpa}"

if __name__ == "__main__":#Тестирует класс
    try:
        student = Student(
            fio="Петров Дима Александрович",
